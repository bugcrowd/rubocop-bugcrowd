...
RSpec.describe RuboCop::Cop::Bugcrowd::DangerousTransaction do
  ...
  it 'registers an offense when using class level method' do
    expect_offense(<<~RUBY)
      ...
      ^^^^^^^^^^^^^^^^^ Use ProperTransaction.start instead of ActiveRecord's base, class, or instance-level transaction methods. See https://bugcrowd.atlassian.net/wiki/spaces/DEV/blog/2020/03/24/1126006819/Learnings+-+Nested+Transactions+in+Rails for more background
    RUBY
  end
